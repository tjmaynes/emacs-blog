...
#+begin_src kotlin
fun <T> getById(id: String): Either<Exception, T?> =
  Either.catch { repository.getById(id) ?: null }
#+end_src

#+begin_src kotlin
val result: Either<Exception, MyObject?> = getById("some-id")

val data = if (result.isRight()) {
  // do something
} else {
  // do something
}

val exception = if (result.isLeft()) {
  // do something else
} else {
  // do something else
}
