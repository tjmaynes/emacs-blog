...
While pairing on my latest project, I did some introspection around the functional programming that I've learned so far. On previous projects, we used a combination of =Either= and =Maybe= monads for abstracting external (IO-based) operations. While I knew how and when to use these tools, I didn't understand where their power came from. After doing some research, I came across this analogy that helped me better understand what a monad is.
...
In our example above, our first line describes a variable of type =Either= whose possible states could be /Error/ or /String/. This is an example of how a monad like an abstract data type. An abstract data type (ADT) is a kind of data type defined by it's behavior from the point of view from a user. Since an ADT is created from the user's point of view, it's internal presentation is hidden. 
...
#+begin_src scala
val logException = (e: Error) => println(s"Unexpected error occurred: $error");

val countries = List[Either[Error, String]] =
  List(
    Right("United States"),
    Left(new Error("State not found: Rusia")),
    Right("United Kingdom"),
    Right("China"),
    Left(new Error("State not found: Paraguy"))
  )
...
** Conclusion
Below is a list of links that helped me when writing this post:
...
- [[https://jrsinclair.com/articles/2016/marvellously-mysterious-javascript-maybe-monad/][James Sinclair's Maybe Monad post]]
- [[https://stackoverflow.com/a/19932439][Monads with Java 8 post]]

Please send me an [[mailto:tj@tjmaynes.com][email]] if I've missed anything in this article. Thank you for reading!
